 ...
 */exports.Element=Element;const eventCallback=function(a,b,c=!0){let d=null;// TODO: Detect if element has [data-action] selector
try{d=!0==c?event.target.closest(`[data-action="${a}"]`):event.target.closest(a),d&&b(d)}catch(a){console.error(`Something went wrong on event callback. Use the correct selector – Didn\'t use the [data-action] selector? Add ${!1} as third argument`)}};/**
...
const b=new Set,c=new FormData(a),d=new Map,e=a.querySelectorAll("input[name], select[name], textarea[name], datalist[name], output[name]");return e.forEach(function(a){const c=returnTag(a),d=a.getAttribute("type");b.add({name:a.getAttribute("name"),type:d?d:c})}),b.forEach(function(a){const b=c.getAll(a.name);"number"==a.type?d.set(a.name,parseFloat(b[0])):"checkbox"==a.type?d.set(a.name,null!=b&&("on"==b||b)):d.set(a.name,b[0])}),d};/**
